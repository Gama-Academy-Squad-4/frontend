...
const Dashboard = () => {
  const { btc24h, consolidado, saldoSemana, saldoMes, dataChart } = useSelector(
    (state) => state.allInfo,
  );
  ...
  return (
    <S.Container>
      ...
      <S.Section1>
        ...
        <S.PanelContainer>
          <Panel1 title="BTC / R$ (24h)" data={panel1A} />
          <Panel1 title="Consolidado" data={panel1B} />
          <Panel2 title="Saldo - Semana" data={panel2A} />

          <Panel2 title="Saldo - Mês" data={panel2B} />
        </S.PanelContainer>
      </S.Section1>
      <S.Section2>
        <S.Title>Posição Consolidada na Semana</S.Title>
        <LocalLineChart data={localDataChart} />
      </S.Section2>
    </S.Container>
  );
};
